...
from config.database import get_database_connection, get_resume_file
import io
import uuid
...
class DashboardManager:
    ...
    def get_skill_distribution(self):
        ...
        cursor = self.conn.cursor()
        cursor.execute("SELECT skills FROM resume_data")
        
        counts = {'Programming': 0, 'Database': 0, 'Cloud': 0, 'Management': 0, 'Other': 0}
        
        for row in cursor.fetchall():
            skills_str = row[0]
            if not skills_str: continue
            
            skills = [s.strip("[]'\" ") for s in skills_str.split(',')]
            for skill in skills:
                if not skill: continue
                s_lower = skill.lower()
                if any(k in s_lower for k in ['python', 'java', 'javascript', 'c++', 'programming', 'react', 'node']):
                    counts['Programming'] += 1
                elif any(k in s_lower for k in ['sql', 'database', 'mongodb', 'postgres', 'mysql']):
                    counts['Database'] += 1
                elif any(k in s_lower for k in ['aws', 'cloud', 'azure', 'gcp', 'docker', 'kubernetes']):
                    counts['Cloud'] += 1
                elif any(k in s_lower for k in ['agile', 'scrum', 'management', 'jira', 'leadership']):
                    counts['Management'] += 1
                else:
                    counts['Other'] += 1
                    
        # Sort and prepare return arrays
        sorted_counts = sorted(counts.items(), key=lambda x: x[1], reverse=True)
        return [x[0] for x in sorted_counts], [x[1] for x in sorted_counts]
    ...
    def get_weekly_trends(self):
        ...
        submissions = []
        for date in dates:
            if self.conn.db_type == 'postgres':
                cursor.execute("""
                    SELECT COUNT(*) 
                    FROM resume_data 
                    WHERE DATE(created_at) = CAST(? AS DATE)
                """, (date,))
            else:
                cursor.execute("""
                    SELECT COUNT(*) 
    ...
    def render_resume_data_section(self):
        ...
        if resume_data:
            ...
            columns = [
                ...
                'ATS Score', 'Keyword Match', 'Format Score', 'Section Score', 'Filename'
            ]
            ...
            with st.container():
                ...
                # Download Resume File Section
                st.markdown("### 📄 Download Original Resume")
                col_down1, col_down2 = st.columns([3, 1])
                with col_down1:
                    resume_id_to_download = st.selectbox("Select Resume ID to Download", options=filtered_df['ID'].unique(), key="resume_dl_select")
                
                with col_down2:
                    if resume_id_to_download:
                        file_data = get_resume_file(int(resume_id_to_download))
                        if file_data and file_data[1]: # valid content
                            filename = file_data[0] or f"resume_{resume_id_to_download}.pdf"
                            st.download_button(
                                label="⬇️ Download File",
                                data=file_data[1],
                                file_name=filename,
                                mime="application/octet-stream",
                                key=f"dl_btn_{resume_id_to_download}"
                            )
                        else:
                             st.warning("No file available")

                # Add download buttons for Excel
    ...
    def render_dashboard(self):
        ...
        st.markdown("""
            ...
            """.format(time.strftime('%B %d, %Y %I:%M %p', time.localtime())), unsafe_allow_html=True)
    ...
    def get_detailed_insights(self):
        ...
        # Most Common Skills
        try:
            cursor.execute("SELECT skills FROM resume_data WHERE skills IS NOT NULL")
            all_skills = cursor.fetchall()
            skill_counts = {}
            for (skills_str,) in all_skills:
                # Clean up the string representation of the list
                cleaned = skills_str.replace('[', '').replace(']', '').replace("'", "").replace('"', '')
            ...
            print(f"Error calculating top skills: {e}")
            top_skills = []
        
        if top_skills:
            skills_parts = [f"{name} ({count} resumes)" for name, count in top_skills]
            skills_text = f"Most in-demand skills: {', '.join(skills_parts)}"
            insights.append({
                'title': 'Top Skills',
                'icon': '💡',
