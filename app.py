...
class ResumeApp:
    def __init__(self):
        ...
        # Load external CSS - Updated 2026-03-12 13:32
        with open('style/style.css') as f:
            css = f.read()
            
        # Add dynamic background image if exists
        import os
        ...
        if os.path.exists(bg_path):
            ...
            css = css.replace(
                'https://raw.githubusercontent.com/Ashwathaz/AI_RESUME-ANALYZER/main/assets/bg.png',
    ...
    def add_footer(self):
        ...
        with col2:
            ...
            <div style='display: flex; justify-content: center; align-items: center; margin-bottom: 10px;'>
                <a href='https://github.com/Ashwathaz' target='_blank' style='text-decoration: none;'>
     ...
     "124852522.jpeg")
        ...
        st.markdown(f"""
            <div class="profile-section">
                <img src="{image_base64 if image_base64 else 'https://avatars.githubusercontent.com/Ashwathaz'}"
                     alt="ASHZ"
                     class="profile-image"
                     onerror="this.onerror=null; this.src='https://avatars.githubusercontent.com/Ashwathaz';">
                <h2 class="profile-name">ASHZ</h2>
                <p class="profile-title">Full Stack Developer & AI/ML Enthusiast</p>
                <div class="social-links">
                    <a href="https://github.com/Ashwathaz" class="social-link" target="_blank">
...
"""
    ...
    def main(self):
        ...
        page_mapping = {name.lower().replace(" ", "_").replace("🏠", "").replace("🔍", "").replace("📝", "").replace("📊", "").replace("🎯", "").strip(): name 
        ...
        if selected:
            cleaned_name = selected.lower().replace(" ", "_").replace("🏠", "").replace("🔍", "").replace("📝", "").replace("📊", "").replace("🎯", "").strip()
            st.session_state.page = cleaned_name

        # Admin Login/Logout section (moved to top expander for access since sidebar is gone)
        with st.expander("🔐 Admin Access (Optional)"):
            if st.session_state.get('is_admin', False):
